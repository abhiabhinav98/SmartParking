		...
		if(session!=null)
		{
			...
			if(park.equals("Cancel Booking"))
			{
				try{
					...
					ResultSet r=s2.executeQuery("select park_num,spot_number,book_in_time,vehicle_type from parking_spot_info where cid='"+username+"'");
					int i=s.executeUpdate("delete from parking_spot_info where cid='"+username+"'");
					r.next();
					pnum=r.getInt(1);
					snum=r.getInt(2);
			...
			else
			{
				try{
					...
					java.sql.Statement s=con.createStatement();
					java.sql.Statement s2=con.createStatement();
					out.print("<header><div class=\"container\"><div class=\"logo pull-left animated wow fadeInLeft\"><img src=\"images/logo.jpg\" height=\"80px\"  width=\"65px\" >Smart Parking Service</div></div></header>");
					//	out.println("<div id=\"text\">");
					...
					vtype=r.getInt(4);
					Float cost;
                                        System.out.println("we are here bhenchod");
					if(vtype == 1 )
					{
					ResultSet r2=s2.executeQuery("select cost_of_car_parkings from parking_lot_info where park_number="+pnum);
					r2.next();
					cost=Float.parseFloat(r2.getString(1));
					}
					else
					{
						ResultSet r2=s2.executeQuery("select cost_of_bike_parkings from parking_lot_info where park_number="+pnum);
						r2.next();
						cost=Float.parseFloat(r2.getString(1));
					}
					Date date=new Date();
